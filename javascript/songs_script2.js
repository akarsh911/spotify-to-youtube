...
function loop()
{
  ...
  if(data.tracks.items[i].track.name!=undefined)
  {
      ...
      if(songindata==0)
      {
          ...
          allsongs='<tr class="row" id="row'+(now+1)+'" onclick="row_click('+(now+1)+');"><th>'+(now+1)+'</th><th><img alt="album image" loading="lazy"  class="imgt" src="'+current_track.album.images[0].url+'"></th><th>'+current_track.name+'</th><th>'+artist+'</th><th>'+link+'</th><th><input class="checkbox" type="checkbox" checked="checked" onclick="check_click('+(now+1)+');" id="chk'+(now+1)+'"/><label for="chk'+(now+1)+'"> </label></th></tr>';
